...
const documentPath = (slug: string) => {
  ...
  return Settings.gitload
    ? `${GitHubLink.href}/raw/main/contents/docs/${filename}`
    : path.join(process.cwd(), "/contents/docs/", filename)
}
...
export async function getTable(
  ...
  if (Settings.gitload) {
    const filename = slug === "" ? "index.mdx" : `${slug}.mdx`
    const contentPath = `${GitHubLink.href}/raw/main/contents/docs/${filename}`
    try {
      const response = await fetch(contentPath)
      if (!response.ok) {
        throw new Error(
    ...
    const filename = slug === "" ? "index.mdx" : `${slug}.mdx`
    const contentPath = path.join(process.cwd(), "/contents/docs/", filename)
    try {
