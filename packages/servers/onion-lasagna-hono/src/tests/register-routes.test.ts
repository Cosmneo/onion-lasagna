/**
 * @fileoverview Integration tests for Hono route registration.
 *
 * Tests the complete flow from unified routes to Hono handlers
 * using Hono's built-in app.request() testing utility.
 */
...
import { registerHonoRoutes } from '../register-routes';
import { onionErrorHandler } from '../error-handler';

describe('Hono register-routes', () => {
  describe('registerHonoRoutes', () => {
    ...
    it('handles PATCH requests with custom response headers', async () => {
      const res = await app.request('/users/789', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
