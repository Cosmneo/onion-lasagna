/**
 * @fileoverview Tests for Zod v4 pagination schemas.
 */

import { describe, it, expect } from 'vitest';
import { z } from 'zod';
import { zodSchema } from '../zod.adapter';
import { pagination } from '../schemas/pagination';

describe('pagination (Zod v4)', () => {
  describe('input', () => {
    it('applies defaults when empty object', () => {
      const schema = zodSchema(pagination.input);
    ...
    it('is composable via .extend()', () => {
      ...
      const schema = zodSchema(extended);
  ...
  describe('response', () => {
    ...
    it('validates valid paginated response', () => {
      const schema = zodSchema(pagination.response(itemSchema));
      const result = schema.validate({
