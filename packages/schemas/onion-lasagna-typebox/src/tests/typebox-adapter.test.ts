...
describe('typeboxSchema', () => {
  describe('validation', () => {
    ...
    it('validates enums', () => {
      ...
      const schema = typeboxSchema(Type.Enum(Status));

      expect(schema.validate('active').success).toBe(true);
      expect(schema.validate('inactive').success).toBe(true);
      expect(schema.validate('unknown').success).toBe(false);
    });
    ...
    it('includes error code in validation issues', () => {
      ...
      const result = schema.validate({ name: 123, age: 'not a number' });
  ...
  describe('JSON Schema generation', () => {
    ...
    it('returns a deep copy to prevent mutation', () => {
      ...
      // Mutate nested properties of the returned schema
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (jsonSchema.properties!.user as any).type = 'array';

      // Original should be unaffected
      const freshSchema = schema.toJsonSchema();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect((freshSchema.properties!.user as any).type).toBe('object');
    });

    it('produces clean JSON Schema without TypeBox metadata', () => {
      const schema = typeboxSchema(Type.Object({ name: Type.String() }));

      const jsonSchema = schema.toJsonSchema();

      // Should not contain Symbol-keyed properties (TypeBox's [Kind], etc.)
