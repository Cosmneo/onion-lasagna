/**
 * @fileoverview Query key generation for Vue Query integration.
 *
 * Builds hierarchical query keys from router configuration.
 * Pure functions with no Vue dependency.
 *
 * @module unified/vue-query/query-keys
 */

import type { RouterConfig, RouterDefinition } from '@cosmneo/onion-lasagna/http/route';
 ...
 *
 * // With prefix to prevent cross-router collisions
 * const keys = buildQueryKeys(router, ['my-api']);
 * keys.users.list()                                   // ['my-api', 'users', 'list']
 * ```
 */
