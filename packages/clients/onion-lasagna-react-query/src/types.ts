 ...
 * Extends the client config with React Query-specific options.
 */
export interface ReactQueryHooksConfig extends ClientConfig {
  /**
   * Prefix prepended to all query keys to prevent cache collisions
   * when multiple `createReactQueryHooks()` instances share the same QueryClient.
   *
   * @example
   * ```typescript
  ...
  /**
   * React hook that gates all generated `useQuery` hooks.
   *
